...
public class DijkstraSP {
    ...
    public DijkstraSP(EdgeWeightedDigraph G, int s, Vector<Integer> blackList) {
        ...
        while (!pq.isEmpty()) {
            int v = pq.delMin();
            for (DirectedEdge e : G.adj(v)){    //{             //My commit: if vertex in black list.
                if( !blackList.contains(e.v) ) //if             //
            	relax(e);                                       //
            }                                   //}             //
        }
    ...
    private boolean check(EdgeWeightedDigraph G, int s) {
        ...
        for (DirectedEdge e : G.edges()) {
            if (e.weight() <= 0) {                                     //My commit: don't want zero(added =).
                System.err.println("negative/zero edge weight detected");
                return false;
            }
        }
    ...
    public static void main(String[] args) {
    	...
    	In in = new In("C:\\Users\\akiva\\Desktop\\tinyEWD.txt");
    	EdgeWeightedDigraph G = new EdgeWeightedDigraph(in);
    	System.out.println(G.toString());
    	//DijkstraSP dijkstra = new DijkstraSP(G, 0);
    	//System.out.println(dijkstra.hasPathTo(2));
